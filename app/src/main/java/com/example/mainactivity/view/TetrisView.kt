...
class TetrisView : View {
    ...
    /*
        Обработчик, обрабатывающий запросы для задержки сообщений и продолжающий работу
        после окончания задержки. Т.к блоки перемещаются вдоль полей с задержкой
     */
    ...
    //Аналогично setGameCommand,но также обновляет счет очков и переводит viewHandler в режим сна
    //после выполнения игровой команды
    ...
    //Унаследован от View. Вызывается, если представление отображает свое содержимое
    ...
    //Вызывается, если изменяется размер представления
